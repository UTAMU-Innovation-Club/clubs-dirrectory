...
const firebaseConfig = {
  apiKey: "AIzaSyDK5mJtEq6b1JiLq1Bb5ZZp9hufYgAgR00",
  authDomain: "utamu-communities.firebaseapp.com",
  databaseURL: "https://utamu-communities-default-rtdb.firebaseio.com",
  projectId: "utamu-communities",
  storageBucket: "utamu-communities.firebasestorage.app",
  messagingSenderId: "391083894574",
  appId: "1:391083894574:web:f9efeb06cf86e35a79fcbf"
};

firebase.initializeApp(firebaseConfig);
const db = firebase.database();
const DB_REF = 'communities';
...
function initFirebase() {
  db.ref(DB_REF).on('value', (snapshot) => {
    ...
    if (data) {
      clubs = Object.keys(data).map(key => ({ id: key, ...data[key] }));
    } else {
      clubs = [];
    }
    renderClubs();
...
window.removeClub = function (clubId) {
  ...
  if (confirm(`Are you sure you want to delete "${target.name}"?`)) {
    db.ref(DB_REF + '/' + clubId).remove()
      .then(() => showToast(`Deleted "${target.name}"`))
      .catch(() => showToast('Error deleting club'));
  }
};
...
if (addClubForm) {
  addClubForm.addEventListener('submit', (e) => {
    ...
    db.ref(DB_REF).push(newClub)
      .then(() => {
        showToast(`Added "${name}" successfully!`);
      })
      .catch(() => {
        showToast('Error adding club');
      });
